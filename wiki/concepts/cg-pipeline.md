---
title: 历史｜CG Pipeline (07.5 step)
tags:
...
Three-layer pipeline that turns `@cg show <name>` LS directives into rendered static images (Phase 1 single-panel + Phase 2 manga multi-panel) and surfaces URLs into `mapping.json`. Phase 3 (AI video gen) is config-gated and deferred to agent-forge.

## Layers

- **Layer A** — `skills/asset-prompt-generator/cg_collector.py` + `llm_shot_splitter.py` (novels-to-lunascript)
  - Scans `*.ls` for `@cg show <name> { duration: ... content: "..." ... }` blocks
...
- **Layer B** — `lunaverse-backend/generate-upscale-matting/cg_render.py`
...
- **Layer C** — `dramatizer/pipeline/cg_{config,stage,mapping,oss_list}.py` (novels-to-lunascript)
...
- LS Go interpreter zero-changes — `cg_show` step already has `url` field bake-in.
...
- Spec: [`novels-to-lunascript/docs/superpowers/specs/2026-05-13-cg-pipeline-design.md`](https://github.com/AugustZAD/novels-to-lunascript/blob/main/docs/superpowers/specs/2026-05-13-cg-pipeline-design.md)
- Plan: [`novels-to-lunascript/docs/superpowers/plans/2026-05-13-cg-pipeline-impl.md`](https://github.com/AugustZAD/novels-to-lunascript/blob/main/docs/superpowers/plans/2026-05-13-cg-pipeline-impl.md)
...
- Layer B: 18 tests in `lunaverse-backend/generate-upscale-matting/tests/test_cg_render.py`
